...
public class FriendsDBAdapter {
	...
	public static final String TAG = "FriendsDBAdapter";
	...
	public static final String CREATE_STRANGER_USERS_TABLE = "create table "
			+ STRANGER_USERS_TABLE + "(id text primary key," + NICK_NAME
			+ " text not null, " + PHONE_NUMBER + " text not null, " + PIC_URL
			+ " text not null, " + USER_JEER_CONTENT + " text not null);";
	...
	class DBHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			String CREATE_TABLE = "create table friendlist"
					...
					+ ",num text not null unique" + ",name text" + ",url text"
					+ ",id text unique);";
	...
	public Cursor findFromNum(String num) {
		Cursor cursor = db.query("friendlist", new String[] { "num", "name",
	...
	public Cursor getAllRows() {
		Cursor cursor = db.query("friendlist", new String[] { "num", "name",
				"url", "id" }, null, null, null, null, null);
		return cursor;
	}

	public Cursor getUsersByNotIn(String selectionArgStr) {
		// 不使用 rawQuery 可能会使得 字段属性为 text 的 where 条件失效，但是 integer 属性的字段没有问题。
		Cursor cursor = db.rawQuery(
				"select * from friendlist where id not in (" + selectionArgStr
						+ ")", null);
		if (cursor == null)
